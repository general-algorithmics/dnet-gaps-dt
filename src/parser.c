...
#endif
    int batch = get_current_batch(net);
    fprintf(stderr, "Saving weights to %s (batch: %d)\n", filename, batch);
...
{
    G_IGNORE_RESULT(fread(l.biases, sizeof(float), l.outputs, fp));
    G_IGNORE_RESULT(fread(l.weights, sizeof(float), l.outputs*l.inputs, fp));
    if(transpose){
    ...
    if (l.batch_normalize && (!l.dontloadscales)){
        G_IGNORE_RESULT(fread(l.scales, sizeof(float), l.outputs, fp));
        G_IGNORE_RESULT(fread(l.rolling_mean, sizeof(float), l.outputs, fp));
        G_IGNORE_RESULT(fread(l.rolling_variance, sizeof(float), l.outputs, fp));
...
{
    G_IGNORE_RESULT(fread(l.scales, sizeof(float), l.c, fp));
    G_IGNORE_RESULT(fread(l.rolling_mean, sizeof(float), l.c, fp));
    G_IGNORE_RESULT(fread(l.rolling_variance, sizeof(float), l.c, fp));
#ifdef GPU
    if(gpu_index >= 0){
...
{
    G_IGNORE_RESULT(fread(l.biases, sizeof(float), l.n, fp));
    if (l.batch_normalize && (!l.dontloadscales)){
        G_IGNORE_RESULT(fread(l.scales, sizeof(float), l.n, fp));
        G_IGNORE_RESULT(fread(l.rolling_mean, sizeof(float), l.n, fp));
        G_IGNORE_RESULT(fread(l.rolling_variance, sizeof(float), l.n, fp));
    }
    int size = l.c*l.size*l.size;
    int i, j, k;
    for(i = 0; i < l.n; ++i){
        float mean = 0;
        G_IGNORE_RESULT(fread(&mean, sizeof(float), 1, fp));
        for(j = 0; j < size/8; ++j){
            int index = i*size + j*8;
            unsigned char c = 0;
            G_IGNORE_RESULT(fread(&c, sizeof(char), 1, fp));
            for(k = 0; k < 8; ++k){
                if (j*8 + k >= size) break;
...
void load_convolutional_weights(layer l, FILE *fp)
{
    ...
    G_IGNORE_RESULT(fread(l.weights, sizeof(float), num, fp));
    if(l.adam){
        G_IGNORE_RESULT(fread(l.m, sizeof(float), num, fp));
        G_IGNORE_RESULT(fread(l.v, sizeof(float), num, fp));
    }
...
void load_weights_upto(network *net, char *filename, int start, int cutoff)
{
    ...
    int minor;
    int revision;
    G_IGNORE_RESULT(fread(&major, sizeof(int), 1, fp));
    G_IGNORE_RESULT(fread(&minor, sizeof(int), 1, fp));
    G_IGNORE_RESULT(fread(&revision, sizeof(int), 1, fp));
    G_IGNORE_RESULT(fread(net->seen, sizeof(int), 1, fp));
    ...
    for(i = start; i < net->n && i < cutoff; ++i){
        ...
        if(l.type == LOCAL){
            ...
            int size = l.size*l.size*l.c*l.n*locations;
            G_IGNORE_RESULT(fread(l.biases, sizeof(float), l.outputs, fp));
            G_IGNORE_RESULT(fread(l.weights, sizeof(float), size, fp));
#ifdef GPU
            if(gpu_index >= 0){
                push_local_layer(l);
            }
