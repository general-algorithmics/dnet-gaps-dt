...
void engine_go(char *filename, char *weightfile, int multi)
{
    ...
    while(1){
        ...
        if (!strcmp(buff, "protocol_version")){
            ...
            int boardsize = 0;
            G_IGNORE_RESULT(scanf("%d", &boardsize));
            //fprintf(stderr, "%d\n", boardsize);
            if(boardsize != 19){
            ...
            int handicap = 0;
            G_IGNORE_RESULT(scanf("%d", &handicap));
            ...
            if(!strcmp(buff, "play"))
            {
                G_IGNORE_RESULT(scanf("%s ", color));
            } else {
                G_IGNORE_RESULT(scanf(" "));
                color[0] = buff[0];
            }
            char c;
            int r;
